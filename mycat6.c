...
int main(int argc, char *argv[]) {
    ...
    // 使用fadvise告诉内核我们的访问模式
    // POSIX_FADV_SEQUENTIAL: 告诉内核我们将顺序读取文件
    // POSIX_FADV_WILLNEED: 告诉内核我们很快就需要这些数据
    if (posix_fadvise(fd, 0, 0, POSIX_FADV_SEQUENTIAL) != 0) {
        // fadvise失败不是致命错误，继续执行
        fprintf(stderr, "Warning: fadvise SEQUENTIAL failed: %s\n", strerror(errno));
    }
    
    if (posix_fadvise(fd, 0, 0, POSIX_FADV_WILLNEED) != 0) {
        // fadvise失败不是致命错误，继续执行
        fprintf(stderr, "Warning: fadvise WILLNEED failed: %s\n", strerror(errno));
    }
    ...
    // 告诉内核我们已经完成了对这个文件的访问
    // POSIX_FADV_DONTNEED: 告诉内核可以丢弃这个文件的页缓存
    if (posix_fadvise(fd, 0, 0, POSIX_FADV_DONTNEED) != 0) {
        // fadvise失败不是致命错误
        fprintf(stderr, "Warning: fadvise DONTNEED failed: %s\n", strerror(errno));
    }
